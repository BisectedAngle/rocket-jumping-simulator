...
# Basically similar to explosion, but a bit weaker
...
@onready var collision_appearance = $Sprite2D
...
func _ready():
	collision_appearance.scale = scale_basedon_scene
	collision_appearance.modulate.a = 1.0 # Beginning alpha (opacity) of the explosion
	
	# Tween the explosion appearance so it fades out over 0.5s
	var tween := create_tween()
	tween.tween_property(collision_appearance, "modulate:a", 0.0, 0.5)
