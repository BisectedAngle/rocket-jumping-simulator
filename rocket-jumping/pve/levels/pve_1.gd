...
@onready var final_win_stats := preload("res://pve/levels/you_win.tscn")

# These manage the enemy counts
@export var max_walkers = 4
@export var max_jumpers = 4
@export var walker_goal = 7
@export var jumper_goal = 7

# These are the possible healing positions
var healing_positions = [Vector2(137,138), Vector2(1800,138), Vector2(137,524), Vector2(1800,524)]
...
# Measures time to complete level
...
func _process(delta: float) -> void:
	...
	# Spawn walker when theres none on screen
	if(walkers==0):
		spawn_walker()
		walker_timer.start()
	
	# Spawn jumper when theres none on screen
	if(jumpers==0):
		spawn_jumper()
		jumper_timer.start()
	
	# Stop spawning walkers when theres maximum number
	if(walkers==max_walkers):
		walker_timer.stop()
	
	# Stop spawning jumpers when theres maximum number
	if(jumpers==max_jumpers):
		jumper_timer.stop()
	
	# When goals are reached, stop the level timer and spawn win screen
	if(walkers_killed>=walker_goal and jumpers_killed>=jumper_goal):
		level_timer.stop()
		
		# Add this elapsed time to global variables so total time can be accumulated
		Global.total_time_taken += elapsed_time
		minutes = int(elapsed_time / 60)
		seconds = int(elapsed_time - (minutes * 60))
		spawn_win_screen()
	
	# Constantly updates the walkers/jumpers killed bars
	update_bars()
...
func update_bars():
	...
	# If enemies killed exceeds goal, then just fix the progress bar and label
	...
	# Using string manipulation to change progress bar label
...
func spawn_win_screen():
	...
	# Updating all the labels in the win_screen scene before display
	...
	# Adding these values to the global variables for the final win screen to use
